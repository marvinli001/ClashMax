...
enum NetworkExtensionRuntimeConstants {
  ...
  static let mihomoSigningIdentifier = "io.github.clashmax.ClashMax.Mihomo"
  // Builds before the universal merge signed one binary per architecture. Kept so a
  // core left over from an older install is still recognised as our own process.
