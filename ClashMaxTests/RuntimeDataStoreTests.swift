...
final class RuntimeDataStoreTests: XCTestCase {
  ...
  /// Discussion #25: the Logs page filtered debug entries back out at quiet levels, so selecting
  /// Debug produced an empty page. The selected level is the only switch.
  func testVisibleLogsHonorSelectedLogLevel() {
    let store = RuntimeDataStore()
    store.appendLog(level: "info", message: "Core ready")
    store.appendLog(level: "debug", message: "controller request body")
    store.flushPendingLogs()

    XCTAssertEqual(store.visibleLogs(logLevel: "info").map(\.message), ["Core ready"])
    XCTAssertEqual(
      store.visibleLogs(logLevel: "debug").map(\.message),
